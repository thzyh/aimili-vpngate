...
def diagnose_openvpn_failure(log_tail: list[str]) -> tuple[int, str]:
    ...
    if "connection reset, restarting" in joined_log or "soft,connection-reset" in joined_log:
        return 2004, "[ERR_OVPN_NODE_UNREACHABLE] 节点连接被远端重置。原因: 远端节点或链路在 OpenVPN 连接期间主动重置，当前候选不可达。"
